// The original source: https://github.com/kokke/tiny-AES-c
/*
...
void* memcpy(void* dest, const void* src, size_t n) {
  ...
  // Word size on the target architecture (4 bytes on 32-bit, 8 bytes on 64-bit)
  const size_t word_size = sizeof(uintptr_t);

  // 1. Copy byte-by-byte until 'dest' is aligned to a word boundary
  while (n > 0 && ((uintptr_t)d % word_size) != 0) {
    *d++ = *s++;
    n--;
  }

  // 2. If both pointers are aligned identically, copy in full machine-word blocks
  if (((uintptr_t)s % word_size) == 0) {
  ...
  // 3. Copy remaining trailing bytes (or all remaining bytes if src/dest were misaligned)
  while (n--) {
    *d++ = *s++;
  }
...
char *test_main(const char *buff, size_t size, char result[64]) {
  // 1. Setup Key and Initialization Vector (IV) - Both 16 bytes for AES-128
  ...
  // 2. Prepare the plaintext input
  const char* raw_string = buff;
  size_t raw_len = size;

  // 3. Apply PKCS#7 Padding
  // CBC requires the buffer length to be an exact multiple of AES_BLOCKLEN (16 bytes).
  uint8_t padding_len = AES_BLOCKLEN - (raw_len % AES_BLOCKLEN);
  ...
  // 5. Decrypt
  // IMPORTANT: AES_CBC_encrypt_buffer modifies the IV stored in `ctx`.
  // You MUST reset the IV before decrypting.
  ...
  uint8_t unpadded_len = padded_len - buffer[padded_len - 1];
  buffer[unpadded_len] = '\0'; // Add null-terminator for string printing
  memcpy(result, buffer, unpadded_len + 1);
  return &result[0];
}
